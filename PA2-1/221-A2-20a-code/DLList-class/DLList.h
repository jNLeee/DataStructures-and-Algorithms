...
public:
  ...
  DLListNode *first_node() const { return header.next; } // return the pointer to the first node
  const DLListNode *after_last_node() const { return &trailer; }  // return the pointer to the trailer
  bool is_empty() const { return header.next == &trailer; }   // return if the list is empty
